...
- If you are running VR/AR, Weather Maker does not support a custom screen space shadow shader due to several show stopping bugs in Unity (I have notified them). Until these are fixed, leave VR/AR as use internal screen space shader or disabled.
...
- For network players, please keep the main camera and audio listener components, but make sure they are disabled. These components are used to detect a local player or remote player object versus some other object.
- The Player prefab object is setup like this if you need an example (Demo/DemoResources/Player.prefab).
- For networking, please see the mirror integration guide near the bottom of this file (search for Unity Networking Replacement)
...
For clouds, extra protection is added with an "AutoCloneProfile" property. Set this to false during play mode and then drag in a new profile to edit clouds during play mode. This ensures profiles are not modified accidently.
...
- Volumetric Clouds:
	- Complete Overview: https://youtu.be/ZoULn0mYbt8
...
WeatherMakerWeatherZoneScript has a FirstTransition static bool that is true to start. If true, the first weather transition is instant. Subsequent transitions are normal. You can reset this bool to true to enable instant transition for the next weather transition.
...
Cloud shadows are only available with volumetric clouds.
...
WeatherMakerNoiseGeneratorScene contains a scene to preview and export 3D textures. WeatherMakerAdvancedNoiseShader.shader and WeatherMakerAdvancedNoiseShaderInclude.cginc contain the shader code used to build the textures.
...
- Vegetation Studio Pro
	...
	- Attach a weather zone to each biome - great for rain forest, snowy mountains, dry desert, etc.
...
- Mirror (Unity Networking Replacement)
	- Ensure Unity 2018.3+ or newer is being used.
	- Ensure .NET script profile is 4.X and .NET standard 2.0 or newer.
	- Download Mirror from asset store: https://assetstore.unity.com/packages/tools/network/mirror-129321?aid=1011lGnL
	- Import Mirror into your project.
	- On your player prefab:
		- Add WeatherMakerMirrorNetworkScript to root of player prefab.
		- Add a script like WeatherMakerNetworkDemoPlayerScript.cs to disable (but not delete) camera and audio listener of network players, as well as adding allowed camera.
		- Make sure your player controller script is checking for local player, like WeatherMakerPlayerControllerScript.cs.
		- See NetworkPlayer prefab for an example.
	- Works basically the same way as UNet did, only less buggy and much faster and with the full source code, these guys are awesome!
	- Try DemoSceneNetwork.
...
- Water with Metal on iOS only works in forward rendering. Deferred rendering has artifacts. I have submitted a bug to Unity as I believe this is a problem in there engine.
