...
namespace DigitalRuby.WeatherMaker
{
    ...
    public class WeatherMakerCommandBufferManagerScript : MonoBehaviour
    {
        ...
        private Camera lastNormalCamera;
        ...
        private void UpdateCommandBuffersForCamera(Camera camera)
        {
            ...
            foreach (WeatherMakerCommandBuffer commandBuffer in commandBuffers)
            {
                ...
                if (camera.stereoEnabled)
                {
                    ...
                    if (commandBuffer.CameraType != WeatherMakerCameraType.Reflection || lastNormalCamera == null)
                    {
                        inverseProj[0] = camera.GetStereoProjectionMatrix(Camera.StereoscopicEye.Left).inverse;
                        inverseProj[1] = camera.GetStereoProjectionMatrix(Camera.StereoscopicEye.Right).inverse;
                    }
                    else
                    {
                        // don't use reflection camera projection, it has a special clipping projection that mucks everything up
                        inverseProj[0] = lastNormalCamera.GetStereoProjectionMatrix(Camera.StereoscopicEye.Left).inverse;
                        inverseProj[1] = lastNormalCamera.GetStereoProjectionMatrix(Camera.StereoscopicEye.Right).inverse;
                    }
                }
                else
                {
                    ...
                    if (commandBuffer.CameraType != WeatherMakerCameraType.Reflection || lastNormalCamera == null)
                    {
                        inverseProj[0] = inverseProj[1] = camera.projectionMatrix.inverse;
                    }
                    else
                    {
                        // don't use reflection camera projection, it has a special clipping projection that mucks everything up
                        inverseProj[0] = inverseProj[1] = lastNormalCamera.projectionMatrix.inverse;
                    }
                }
                ...
                {
                    commandBuffer.ReprojectionState.PreRenderFrame(camera, lastNormalCamera);
                }
