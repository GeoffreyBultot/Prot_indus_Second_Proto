...
namespace DigitalRuby.WeatherMaker
{
    ...
    public class WeatherMakerWeatherZoneScript : MonoBehaviour
    {
        ...
        [MinMaxSlider(0.0f, 600.0f, "Override random duration for profiles to transition in (set to 0 to not override)")]
        public RangeOfFloats TransitionDuration = new RangeOfFloats { Minimum = 0, Maximum = 0 };

        [MinMaxSlider(0.0f, 600.0f, "Override random duration for profiles to hold before transition to another profile (set to 0 to not override)")]
        ...
        /// <summary>
        /// Whether the next transition is the very first, if so it will run without animation. Auto sets to false when first transition happens. You can reset to true to enable instant transition again.
        /// </summary>
        public static bool FirstTransition = true;
        ...
        {
            if (WeatherMakerScript.IsServer && WeatherMakerScript.IsPlayer(other.transform))
            {
        ...
        private void OnTriggerExit(Collider other)
        {
            ...
            else if (WeatherMakerScript.IsServer && WeatherMakerScript.IsPlayer(other.transform))
            {
        ...
        private void TransitionFrom(TriggerEnterEvent evt)
        {
            ...
            string clientId = WeatherMakerScript.GetClientId(evt.Transform);
            ...
            WeatherMakerScript.Instance.RaiseWeatherProfileChanged(previousProfile, currentScript.currentProfile, transitionDuration * (previousScript == null ? 1.0f : TransitionDurationMultiplier),
                secondsRemainingHold, WeatherMakerScript.IsServer, (clientId == null ? null : new string[1] { clientId }));
        }
        ...
        private void NotifyThoseInZoneOfProfileChange(WeatherMakerProfileScript oldProfile, WeatherMakerProfileScript newProfile, float transitionDuration)
        {
            ...
            foreach (var kv in activeZones)
            {
                ...
                if (kv.Value.Stack.Count != 0 && kv.Value.Stack[kv.Value.Stack.Count - 1] == this)
                {
                    ...
                    // find a client id
                    string clientId = WeatherMakerScript.GetClientId(kv.Value.Transform);
                    if (clientId != null)
                    {
                        // found client id, add to list
                        clientIds = (clientIds ?? new List<string>());
            ...
            if (foundOne)
            {
                ...
                WeatherMakerScript.Instance.RaiseWeatherProfileChanged(oldProfile, newProfile, secondsRemainingTransition,
                    secondsRemainingHold, WeatherMakerScript.IsServer, (clientIds == null ? null : clientIds.ToArray()));
            }
        }
        ...
        private void UpdateProfile()
        {
            ...
            if (WeatherMakerScript.Instance == null || !WeatherMakerScript.IsServer || (SingleProfile == null && ProfileGroup == null))
            {
                return;
            }
            else if (SingleProfile == null)
            {
                ...
                if (secondsRemainingTransition <= 0.0f)
                {
                    if ((secondsRemainingHold -= Time.deltaTime) <= 0.0f)
                    {
                        ...
                        RangeOfFloats duration = (FirstTransition ? new RangeOfFloats(0.01f, 0.01f) : TransitionDuration);
                        currentProfile = WeatherMakerProfileGroupScript.OverrideProfile(currentProfile, CloudProfile, PrecipitationProfile, FogProfile, WindProfile, LightningProfile, SoundProfile, duration, HoldDuration);
                        secondsRemainingTransition = (FirstTransition ? 0.0f : currentProfile.RandomTransitionDuration());
            ...
            else if (SingleProfile != currentProfileSingle)
            {
                ...
                WeatherMakerProfileScript oldProfile = currentProfile;
                RangeOfFloats duration = (FirstTransition ? new RangeOfFloats(0.01f, 0.01f) : TransitionDuration);
                currentProfile = WeatherMakerProfileGroupScript.OverrideProfile(SingleProfile, CloudProfile, PrecipitationProfile, FogProfile, WindProfile, LightningProfile, SoundProfile, duration, HoldDuration);
                secondsRemainingTransition = (FirstTransition ? 0.0f : currentProfile.RandomTransitionDuration());
                FirstTransition = false;
