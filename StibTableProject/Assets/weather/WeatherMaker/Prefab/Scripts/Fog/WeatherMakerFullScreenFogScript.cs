...
namespace DigitalRuby.WeatherMaker
{
    public class WeatherMakerFullScreenFogScript : WeatherMakerFogScript<WeatherMakerFullScreenFogProfileScript>
    {
        ...
        private void UpdateUnityFog()
        {
            ...
            RenderSettings.fog = (FogDensity > 0.0001f && FogProfile.FogMode != WeatherMakerFogMode.None);
            Color fogColor = FogProfile.FogColor * WeatherMakerLightManagerScript.GetGradientColorForSun(FogProfile.FogGradientColor);
            if (WeatherMakerLightManagerScript.Instance != null)
            {
                foreach (WeatherMakerLightManagerScript.LightState light in WeatherMakerLightManagerScript.Instance.Lights)
                {
                    ...
                    {
                        fogColor *= (light.Light.color * light.Light.intensity);
                        break;
                    }
                }
            }
        ...
        private void UpdateFogProperties()
        {
            ...
            effect.SetupEffect(FogMaterial, FogFullScreenMaterial, FogBlurMaterial, BlurShader, DownSampleScale, WeatherMakerDownsampleScale.Disabled,
