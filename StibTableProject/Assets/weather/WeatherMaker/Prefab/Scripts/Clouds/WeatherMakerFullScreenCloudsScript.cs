...
namespace DigitalRuby.WeatherMaker
{
    public class WeatherMakerFullScreenCloudsScript : MonoBehaviour
    {
        ...
        private static float lastSeed = 0.0f;
        ...
        /// Cloud weather map
        /// </summary>
        internal RenderTexture weatherMapRenderTexture;
        ...
        private void GenerateWeatherMap(Camera camera)
        {
            ...
            if (weatherMapRenderTexture != null)
            {
                ...
                if (currentRenderCloudProfile.WeatherMapRenderTextureOverride == null)
                {
            ...
            currentRenderCloudProfile.UpdateWeatherMap(WeatherMapMaterial, camera);
        }
        ...
        private void UpdateShaderProperties(WeatherMakerCommandBuffer b)
        {
            ...
            {
                Shader.SetGlobalInt(WMS._WeatherMakerAuroraSampleCount, 0);
            }
            else
            {
            ...
            if (sampleCount <= 0 ||
                ...
                WeatherMakerLightManagerScript.Instance.Sun.ViewportPosition.z <= 0.0f ||
        ...
        {
            if (WeatherMakerLightManagerScript.Instance == null || WeatherMakerLightManagerScript.Instance.Sun == null || CloudProfile == null)
            {
                return;
            }
            LensFlare flare = WeatherMakerLightManagerScript.Instance.Sun.GetComponent<LensFlare>();
            if (flare == null)
            {
                return;
            }
            ...
            else
            {
                ...
                Vector3 toSun = (c.transform.position - WeatherMakerLightManagerScript.Instance.Sun.transform.position).normalized;
        ...
        private void DeleteAndTransitionRenderProfile(WeatherMakerCloudProfileScript newProfile)
        {
            if (newProfile != null)
            {
                if (newProfile.name.IndexOf("(Clone)") < 0 && AutoCloneCloudProfileOnChange)
                {
            ...
            if (currentRenderCloudProfile != null)
            {
                ...
                if (currentRenderCloudProfile != emptyProfile)
                {
                    ...
                    {
                        Destroy(currentRenderCloudProfile.CloudLayer1);
                    }
                    if (currentRenderCloudProfile.CloudLayer2.name.IndexOf("(Clone)") >= 0)
                    {
                        Destroy(currentRenderCloudProfile.CloudLayer2);
                    }
                    if (currentRenderCloudProfile.CloudLayer3.name.IndexOf("(Clone)") >= 0)
                    {
                        Destroy(currentRenderCloudProfile.CloudLayer3);
                    }
                    if (currentRenderCloudProfile.CloudLayer4.name.IndexOf("(Clone)") >= 0)
                    {
                        Destroy(currentRenderCloudProfile.CloudLayer4);
                    }
                    if (currentRenderCloudProfile.CloudLayerVolumetric1.name.IndexOf("(Clone)") >= 0)
                    {
                        Destroy(currentRenderCloudProfile.CloudLayerVolumetric1);
                    }
                    if (currentRenderCloudProfile.name.IndexOf("(Clone)") >= 0)
                    {
                        Destroy(currentRenderCloudProfile);
                    }
                }
            }
            if (newProfile != null && AutoCloneCloudProfileOnChange)
            {
                currentRenderCloudProfileIsTemporary = true;
            }
            currentRenderCloudProfile = _CloudProfile = lastProfile = newProfile;

            if (currentRenderCloudProfile != null && currentRenderCloudProfileIsTemporary && newProfile.CloudLayerVolumetric1.CloudCover.Maximum > 0.0f)
            {
                ...
                {
                    currentRenderCloudProfile.CloudLayer1.CloudCover = 0.0f;
                }
                ...
                {
                    currentRenderCloudProfile.CloudLayer2.CloudCover = 0.0f;
                }
                ...
                {
                    currentRenderCloudProfile.CloudLayer3.CloudCover = 0.0f;
                }
                ...
                {
                    currentRenderCloudProfile.CloudLayer4.CloudCover = 0.0f;
                }
            }
        }
        ...
        private void EnsureProfile()
        {
            ...
            else if (AutoCloneCloudProfileOnChange && _CloudProfile.name.IndexOf("(Clone)", StringComparison.OrdinalIgnoreCase) < 0)
            {
        ...
        private void CreateWeatherMap()
        {
            ...
            {
                WeatherMakerFullScreenEffect.DestroyRenderTexture(ref weatherMapRenderTexture);
                int size = (WeatherMakerScript.Instance == null ? 512 : WeatherMakerScript.Instance.PerformanceProfile.VolumetricCloudWeatherMapSize);
                ...
                weatherMapRenderTexture.autoGenerateMips = false;
                weatherMapRenderTexture.antiAliasing = 1;
                weatherMapRenderTexture.anisoLevel = 1;
                weatherMapRenderTexture.useMipMap = false;

                weatherMapRenderTexture.Create();

                if (lastSeed == 0.0f || WeatherMapRegenerateSeedOnEnable)
                {
                    Shader.SetGlobalFloat(WMS._WeatherMakerWeatherMapSeed, (lastSeed = UnityEngine.Random.Range(0.001f, 128.0f)));
                }
        ...
        {
            currentRenderCloudProfile.SetShaderCloudParameters(material, camera);
            if (WeatherMakerLightManagerScript.Instance != null && cloudCollider != null && currentRenderCloudProfile != null && currentRenderCloudProfile.CloudsEnabled)
            {
                WeatherMakerLightManagerScript.Instance.UpdateShaderVariables(camera, material, cloudCollider);
            }
        }
        ...
        private void CameraPreRender(Camera camera)
        {
            if (weatherMapRenderTexture != null)
            {
                Shader.SetGlobalTexture(WMS._WeatherMakerWeatherMapTexture, weatherMapRenderTexture);
            }
            if (effect != null && !WeatherMakerScript.ShouldIgnoreCamera(this, camera, !AllowReflections))
            {
            ...
            if (WeatherMakerScript.GetCameraType(camera) == WeatherMakerCameraType.Normal &&
                camera.stereoTargetEye != StereoTargetEyeMask.Right && camera.stereoActiveEye != Camera.MonoOrStereoscopicEye.Right)
            {
