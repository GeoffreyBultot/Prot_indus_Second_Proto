...
{
    [CreateAssetMenu(fileName = "WeatherMakerDayNightCycleProfile", menuName = "WeatherMaker/Day-Night Cycle Profile", order = 90)]
...
#pragma warning restore 0414
        ...
        private void SetCelestialObjectPosition(WeatherMakerCelestialObjectScript obj, Vector3 transformForward)
        {
            switch (obj.OrbitType)
            {
                case WeatherMakerOrbitType.Custom:
                    ...
                    if (orbitInterface == null)
                    {
                        Debug.LogErrorFormat("Unable to use custom orbit for celestial object {0}, field OrbitTypeCustomScript" +
                            " must be set to a MonoBehavior script implementing IWeatherMakerCelestialObjectCustomOrbit", obj.gameObject.name);
                    }
                    else
                    {
                ...
                default:
                    // TODO: Causes jarring transition, fix this sometime...
        ...
        private void UpdateSun(CameraMode mode, WeatherMakerCelestialObjectScript sun, float l)
        {
            if (sun == null || sun.Light == null)
            {
                return;
            }
            else if (mode == CameraMode.OrthographicXY)
            {
        ...
        {
            float dot, yPower;
            float sunIntensityReducer = (sun == null || sun.Light == null ? 1.0f : 1.0f - Mathf.Min(1.0f, Mathf.Pow(sun.Light.intensity, 0.2f)));
            ...
            for (int i = 0; i < moons.Count; i++)
            {
                ...
                moon.Light.intensity = moon.LightBaseIntensity * yPower * sunIntensityReducer * (float)MoonDatas[i].PercentFull * (float)MoonDatas[i].PercentFull * dot * moon.LightMultiplier;
        ...
        {
            if (WeatherMakerLightManagerScript.Instance == null || WeatherMakerLightManagerScript.Instance.Sun == null || WeatherMakerLightManagerScript.Instance.Sun.transform == null)
            {
                return;
            }
            ...
            if (accumulatedTime > UpdateInterval)
            {
                ...
                WeatherMakerCelestialObjectScript sun = WeatherMakerLightManagerScript.Instance.Sun;
                IList<WeatherMakerCelestialObjectScript> moons = WeatherMakerLightManagerScript.Instance.Moons;
                float l = WeatherMakerLightManagerScript.GetGradientLookupValueSun();
                UpdateAmbientColors(l);
                UpdateTimeOfDay(updateTimeOfDay);
                UpdateSun(mode, sun, l);
