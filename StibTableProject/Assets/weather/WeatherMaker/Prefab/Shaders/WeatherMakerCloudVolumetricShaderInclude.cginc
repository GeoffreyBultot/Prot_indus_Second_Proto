...
float4 CloudVolumetricSampleWeather(float3 pos);
...
static const float volumetricCameraHeightFrac = GetCloudHeightFractionForPoint(_CloudCameraPosition);
static const float4 volumetricCameraWeatherData = CloudVolumetricSampleWeather(_CloudCameraPosition);
static const float volumetricCameraCloudDensity = SampleCloudDensity(_CloudCameraPosition, volumetricCameraWeatherData, volumetricCameraHeightFrac, 0.0, true);
static const float volumetricMinRayY = lerp(_CloudMinRayYVolumetric, -1.0, _CloudCameraPosition.y >= _CloudStartVolumetric);
...
float SampleCloudDensityDetails(float noise, float3 marchPos, float heightFrac, float lod)
{
	...
	if (volumetricPositionDetailScale > 0.0 && noise > VOLUMETRIC_MIN_NOISE_VALUE)
	{
		...
		*/
		...
		noise = min(1.0, noise * _CloudNoiseScalarVolumetric);
	}
...
float SampleCloudDensity(float3 marchPos, float4 weatherData, float heightFrac, float lod, bool sampleDetails)
{
		...
		(heightGradientSingle = GetDensityHeightGradientForHeight(heightFrac, cloudType)) > VOLUMETRIC_MIN_NOISE_VALUE)
	{
		...
		*/
		...
		if (sampleDetails)
		{
			noise = SampleCloudDensityDetails(noise, marchPos, heightFrac, lod);
		}
	}
...
fixed3 SampleDirLightSources(float3 marchPos, float3 rayDir, float startHeightFrac, float cloudSample, float eyeDensity, float lod)
{
	...
	for (uint lightIndex = 0; lightIndex < uint(_WeatherMakerDirLightCount) && _WeatherMakerDirLightColor[lightIndex].a > 0.0; lightIndex++)
	{
		...
		for (uint i = 0.0; i < volumetricLightIterations; i++)
		{
			...
			weatherData = CloudVolumetricSampleWeather(samplePos);
        ...
        if (volumetricLightSampleDistant)
        {
            ...
            pos += (lightStep * 9.0);
            weatherData = CloudVolumetricSampleWeather(pos);
	...
	inout uint marches, inout float opticalDepth)
{
	...
	{
		uint i = 0;
		bool blockedByDepthBuffer = (distance(marchPos, _CloudCameraPosition) >= depth);
		...
		float marchLength = clamp(min(volumetricCloudMaxRayLength, rayLength) * invSampleCount, VOLUMETRIC_MIN_STEP_LENGTH, VOLUMETRIC_MAX_STEP_LENGTH);
		...
		float startLod = volumetricLod.x + max(0.0, (opticalDepth * VOLUMETRIC_LOD_OPTICAL_DEPTH_MULTIPLIER) - 1.0) + lodIncreaser;
		...
		while (i++ < sampleCount && cloudColor.a < 0.999 && heightFrac > -0.001 && heightFrac < 1.001)
		{
			...
			weatherData = CloudVolumetricSampleWeather(marchPos);
			...
			if (CloudVolumetricGetCoverage(weatherData) > VOLUMETRIC_MINIMUM_COVERAGE_FOR_CLOUD)
			{
				...
				if (cloudSample > 0.0)
				{
					...
					cloudSample = SampleCloudDensityDetails(cloudSample, marchPos, heightFrac, lod);
			...
			UNITY_BRANCH
			if (_CloudRaymarchSkipThreshold != 0 && !inCloud && ++zeroSampleCounter > _CloudRaymarchSkipThreshold)
			{
				...
				marchDir = lerp(marchDir, marchDir * _CloudRaymarchSkipMultiplier, zeroSampleCounter2 < _CloudRaymarchSkipMultiplierMaxCount);
				lastZeroPos = marchPos;
				zeroSampleCounter2++;
			}
...
fixed4 ComputeCloudColorVolumetric(float3 rayDir, float4 uv, float depth)
{
	...
	uint iterationIndex;
	uint lightIndex;
	uint iterations = SetupPlanetRaymarch(_CloudCameraPosition, cloudRayDir, 10000000.0, depth, volumetricSphereInner, volumetricSphereOutter,
	...
	{
		skyColor = lerp(backgroundSkyColor * _CloudSkyIntensityVolumetric, volumetricCloudAmbientColorSky, min(1.0, volumetricCameraCloudDensity * 3.0));
...
float ComputeCloudShadowStrength(float3 worldPos, uint dirIndex)
{
	...
	else
	{
		...
		if (_WeatherMakerShadowsEnabled && shadowValue > 0.0 && _CloudCoverVolumetric > 0.01 && dirIndex < uint(_WeatherMakerDirLightCount) && _WeatherMakerDirLightColor[dirIndex].a > 0.0)
		{
			...
			float3 cloudRayDir = normalize(float3(rayDir.x, rayDir.y + _CloudRayOffsetVolumetric, rayDir.z));
			SetupPlanetRaymarch(worldPos, cloudRayDir, 10000000.0, 10000000.0, volumetricSphereInner, volumetricSphereOutter,
			...
			if (rayLength > 0.01)
			{
				...
				float4 weatherData = CloudVolumetricSampleWeather(marchPos);
				float cloudType = CloudVolumetricGetCloudType(weatherData);
