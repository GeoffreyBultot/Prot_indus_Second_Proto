...
Shader "WeatherMaker/WeatherMakerCloudShadowShader"
{
	Properties
	{
		...
		_CloudShadowScale("Scale", Range(0.01, 100.0)) = 10.0
	}
	SubShader
	{
		...
		#define VOLUMETRIC_CLOUD_SHADOW_DITHER 0.002
		...
		uniform float _CloudShadowScale;
		...
		Pass
		{
			...
			float4 shadowFrag(full_screen_fragment i) : SV_Target
			{
				...
				if (depth < 1.0)
				{
					float3 worldPos = _CloudCameraPosition + (depth * i.forwardLine);
					return ComputeCloudShadowStrength(worldPos * _CloudShadowScale, 0);
				}
				else
				{
					return 1.0;
		...
		Pass
		{
			...
			float shadowFrag(full_screen_fragment i) : SV_DEPTH
			{
				...
				WM_INSTANCE_FRAG(i);
				float3 worldPos = _CloudCameraPosition + (GetDepth01(i.uv) * i.forwardLine);
