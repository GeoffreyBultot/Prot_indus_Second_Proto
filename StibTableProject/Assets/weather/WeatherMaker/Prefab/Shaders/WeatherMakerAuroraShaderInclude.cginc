...
// are copyright (c) 2019 Digital Ruby, LLC and must not be copied, redistributed or reproduced without express written consent
...
uniform float _WeatherMakerAuroraShapeScale;
uniform float _WeatherMakerAuroraDetailScale;
...
// pos zw is the detail position
float auroraLineNoise(inout float4 pos)
{
	// TODO: There are some things that are not ideal - this noise function is VERY simplistic and fast
	// which is great for performance, not so great for super realistic aurora borealis
	// in the future, a formula or modifier to the noise that forces larger and more dramatic curves would be ideal
	...
	// how much should the detail position noise effect the shape pos noise
	// a value of 0 means no influence and basically no randomness
	// a value of 1 means full influence
	// values higher than 1 tile badly, so not recommended
	const float detailPosNoiseMultiplier = 0.69; // strangely this value seems to look the best in my opinion, coincedence?
	...
	// I tried adding additional jagged and curved lines, but things go too noisy
	...
	// scale detail noise
	detailPosNoise *= detailPosNoiseMultiplier;
	...
	// compute shape noise from the shape position, basically the same way as the detail noise was computed
	// the shape noise is the general structure of the noise
...
float auroraLineNoiseFbm(float2 pos)
{
	...
	const float shapePosMultiplier = _WeatherMakerAuroraShapeScale;
	const float detailPosMultiplier = _WeatherMakerAuroraDetailScale; // lower for less detail, raise for more detail
	const float octaveDecay = 0.45; // how much noise should decay at each octave, starting with octaveAmp

	// vars
	float noise = 0.0; // accumulate noise, since this is fbm style loop
	float octaveAmp = 0.72; // multiply final noise by this value each octave, reduces by octaveDecay each octave
...
fixed4 ComputeAurora(float3 rayOrigin, float3 rayDir, float2 uv, float depth)
{
	...
	float4 auroraSphereInner = float4(rayOrigin.x, -AURORA_PLANET_RADIUS, rayOrigin.z, innerRadius);
	float4 auroraSphereOutter = float4(rayOrigin.x, -AURORA_PLANET_RADIUS, rayOrigin.z, outterRadius);
	...
	for (uint iteration = 0; iteration < iterations; iteration++)
	{
		...
		// march while we are inside the aurora box
		UNITY_LOOP
		for (i = 0; i < _WeatherMakerAuroraSampleCount && color.a < 0.99 && heightFrac > -0.01 && heightFrac < 1.01; i++)
		{
			// get away from 0,0, the singularity is near!
			scaledPos = (pos.xz + float2(65536.12323, 32768.653234)) * auroraMarchScale.xz;

			// sample the aurora at the current position
			noiseValue = auroraMapNoiseValue(auroraLineNoiseFbm(scaledPos));

			// compute height frac, the percentage to the top of the aurora plane we are, just like volumetric clouds
			//heightFrac = saturate(((pos.y - _WeatherMakerAuroraHeight.x) * invAuroraHeight));
	...
	// premultiply alpha, add intensity
	return fixed4(color.rgb * color.a * auroraIntensity, color.a);
}
