...
uniform float3 _CloudCameraPosition;
...
float3 CloudRaycastWorldPosPlane(float3 ray, float depth)
{
	...
	float planeMultiplier = RayPlaneIntersect(_CloudCameraPosition, ray, float3(0.0, 1.0, 0.0), planePos, distanceToPlane);
	planeMultiplier *= (depth > distanceToPlane);
	float3 intersectPos = _CloudCameraPosition + (ray * distanceToPlane);
