		...
		Pass
		{
			...
			{ 
				...
				if (_CloudCoverVolumetric > 0.0)
				{
					...
					uint iterations = SetupPlanetRaymarch(_CloudCameraPosition, cloudRayDir, depth, depth, volumetricSphereInner, volumetricSphereOutter,
						startPos, rayLength, distanceToSphere, startPos2, rayLength2, distanceToSphere2);
					return lerp(saturate(distanceToSphere * _ProjectionParams.w), 1.0, (iterations == 0 || rayLength < 0.1));
				}
				else
