...
fixed3 ComputeDirectionalLightFog(float3 rayOrigin, float3 rayDir, float rayLength, float fogFactor, float2 screenUV)
{
	...
	fixed3 baseScatter = (fogFactor * (sunLightColor + (sunLightColor * scatter)));

	if (WM_FOG_VOLUMETRIC_LIGHT_MODE_ENABLE_WITH_SHADOWS && WM_CAMERA_RENDER_MODE_NORMAL && _WeatherMakerFogLightShadowBrightness > 0.0)
	{
	...
	{
		UNITY_BRANCH
		if (_WeatherMakerDirLightDirection[i].w == 1.0)
		{
...
fixed4 ComputeFogLighting(float3 rayOrigin, float3 rayDir, float rayLength, float fogFactor, float2 screenUV, float noise)
{
	...
	else
	{
		...
		lightColor.rgb += ComputeDirLightShaftColor(screenUV, fogFactor, _WeatherMakerDirLightViewportPosition[0], _WeatherMakerFogColor);
		lightColor.a = fogFactor;
		lightColor.rgb += (noise * _WeatherMakerFogEmissionColor.rgb); // _WeatherMakerFogEmissionColor.a is already built into the shader property
