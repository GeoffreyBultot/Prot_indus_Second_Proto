...
#define SAMPLE_DEPTH_4_01(source, i, taps) \
		float depth1 = Linear01Depth(UNITY_SAMPLE_DEPTH(SAMPLE_DEPTH_TEXTURE(source, i.uv.xy + taps[0]))); \
		float depth2 = Linear01Depth(UNITY_SAMPLE_DEPTH(SAMPLE_DEPTH_TEXTURE(source, i.uv.xy + taps[1]))); \
		float depth3 = Linear01Depth(UNITY_SAMPLE_DEPTH(SAMPLE_DEPTH_TEXTURE(source, i.uv.xy + taps[2]))); \
		float depth4 = Linear01Depth(UNITY_SAMPLE_DEPTH(SAMPLE_DEPTH_TEXTURE(source, i.uv.xy + taps[3])));
...
#error Invalid downsample mode
		...
		{
			WM_INSTANCE_FRAG(i);
			return Linear01Depth(UNITY_SAMPLE_DEPTH(SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, i.uv)));
		}
