...
uniform float4 _WeatherMakerDirLightDirection[MAX_LIGHT_COUNT]; // from dir light
...
uniform float _WeatherMakerDirLightDifference; // how much have dir lights changed since the last frame
...
uniform float _WeatherMakerPointLightDifference[MAX_LIGHT_COUNT];
...
uniform float _WeatherMakerSpotLightDifference[MAX_LIGHT_COUNT];
...
uniform float _WeatherMakerAreaLightDifference[MAX_LIGHT_COUNT];
...
fixed3 ComputeDirLightShaftColor(float2 screenUV, fixed fogFactor, float4 lightViewportPos, fixed3 shaftColor)
{
	...
	UNITY_BRANCH
	if (WM_FOG_SUN_SHAFT_ENABLE && WM_CAMERA_RENDER_MODE_NORMAL && _WeatherMakerDirLightCount > 0 && _WeatherMakerFogSunShaftsParam1.y > 0)// && lightViewportPos.z >= 0.0)
	{
		...
		float2 uvMarch = (viewportPos - screenUV) * _WeatherMakerFogSunShaftsParam1.x;
		...
		color *= shaftColor * _WeatherMakerFogSunShaftsParam1.w * _WeatherMakerFogSunShaftsTintColor.rgb * lightViewportPos.w * shaftBrightness * fogFactorReducer1 * fogFactorReducer2;
	}
