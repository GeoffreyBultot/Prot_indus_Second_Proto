	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 full_screen_overlay_fragment_shader(full_screen_fragment i) : SV_Target
			{
				...
				float3 gBufferNormal = tex2Dlod(_CameraGBufferTexture2, float4(i.uv.xy, 0.0, 0.0)).xyz;
				...
				float depth = WM_SAMPLE_DEPTH_DOWNSAMPLED_01(i.uv);
				...
				float depth;
				float3 normal;
				GetDepth01AndNormal(i.uv, depth, normal);
				...
				{
					wpos = _WorldSpaceCameraPos + (rayDir * depth * _ProjectionParams.z);
				}
				else
				{
					wpos = _WorldSpaceCameraPos + (depth * i.forwardLine);
				}
				return ComputeOverlayColor(wpos, rayDir, normal, depth, i.uv);
			}
